...
void Game::render(const uint64_t& start)
{
    ...
    auto elapsed_s = (m_end - start) ;


     std::string s="";
     s+= std::to_string(static_cast<int>(1.0/(elapsed_s / 1000.f)));
     s+='\0';
    ...
    printf("#################### next frame\n");
    printf(" time per frame %f in ms\n", elapsed_s);
    printf(" time per frame %f in s \n", elapsed_s / 1000.f);
    printf(" frame per second %f  \n", (1.f)/(elapsed_s / 1000.f));
...
{     
    ...
    m_time_passed_texture.free();
    m_prompt_texture.free();

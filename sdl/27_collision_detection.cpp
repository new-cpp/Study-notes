...
struct Entity
{
    ...
    bool check_collision(const SDL_Rect& box);    

    int m_pos_x{},m_pos_y{};
    int m_velocity_x{},m_velocity_y{};

    Texture_manager m_dot_texture;
    SDL_Rect m_bound_box{0,0,dimension_x, dimension_y};
};
...
void Entity::move(const SDL_Rect& wall)
{
   m_pos_x += m_velocity_x;
   m_pos_y += m_velocity_y;

   m_bound_box.x = m_pos_x;
   m_bound_box.y = m_pos_y;

   if(m_pos_x <0 || m_pos_x + dimension_x > SCREEN_WIDTH || check_collision(wall))
   {
       m_pos_x -= m_velocity_x;
       m_bound_box.x = m_pos_x;
   }
...
bool Entity::check_collision(const SDL_Rect& box)
{
    SDL_Rect a = m_bound_box;
    SDL_Rect b = box;
...
void Game::update()
{
    ...
    //just now it's bad evey time to constuct sdl_rect
   m_dot.move(m_wall);
...
void Game::render(const uint64_t& start)
{
    ...
    SDL_RenderDrawRect(m_renderer,&m_wall); 
